...
class Discogs extends React.Component {
    ...
    searchData = () => {
        ...
        let url = 'https://api.discogs.com/database/search?key=DmBVubZaVUOdLNgAaKpl&secret=QbEBcLYRLDjQSHZUWkuDODjrqzFAbyxY&artist='
                    ...
                    if (response.ok) {
                        ...
                        response.json().then(json_response => {
                            ...
                            this.setState({
                                ...
                                //wind_speed:json_response.description,
    ...
    render() {
        // alert(this.state.isChanged+''+this.state.isadded)
                ...
                for (let i = 0; i < this.state.tracks_data.results.length; i++) {
                    array_data.push(
                        <tr key={i}>
                            ...
                            <td>{this.playlistOptions()} <button onClick={() => this.addBanda(this.state.tracks_data.results[i].id)}>Add</button></td>
                        </tr>
                    )
                }
                return (
                    <div className="panel350 ">
                        <input type="text" id="artist" placeholder="Enter Indian Artist Name.." />
